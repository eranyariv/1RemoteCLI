...
The hub keeps two small files under `$HOME/data` — no database, nothing that needs a migration. The [operator channel's](operator-channel.md) counters are one; the other is project definitions (issue #110): `$HOME/data/1RemoteCLI/project-state.json` (name, description, URLs, icon version — one array per user, `General` auto-seeded) plus a sibling `project-icons/` directory of the actual uploaded icon bytes, keyed by user then project id. Both are versioned and tolerant of being read by older or newer code, and both are rewritten atomically (temp file + rename) so a crash mid-write can never corrupt them — worst case on a bad read is starting that one file empty (a user's projects re-seed to just `General`), not failing to boot. Unlike the operator channel's 30-second flush timer, project writes flush to disk immediately, since they are rare and user-initiated. Configurable the same way, via `Projects:StatePath` / `Projects:IconRoot` in app settings, should the default `$HOME/data` location ever need to move. Rolling back is just running the older code — session-to-project assignment itself isn't in this file at all (it lives in memory beside the existing session rename/pin labels), so at most a rollback loses a live session's project assignment, not the projects themselves. Sessions survive it the same way they survive a restart: agents and phones reconnect within a few seconds.
