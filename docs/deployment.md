...
The hub keeps two small files under `$HOME/data` — no database, nothing that needs a migration. The [operator channel's](operator-channel.md) counters are one; the other is project state (issue #110): `$HOME/data/1RemoteCLI/project-state.json` (project fields plus live session-to-project assignments, partitioned per user, with `General` auto-seeded) plus a sibling `project-icons/` directory of uploaded icon bytes, keyed by user then project id. Both are versioned and tolerant of being read by older or newer code, and both are rewritten atomically (temp file + rename) so a crash mid-write can never corrupt them — worst case on a bad read is starting that one file empty (a user's projects re-seed to just `General`), not failing to boot. Unlike the operator channel's 30-second flush timer, project writes flush to disk immediately, since they are rare and user-initiated. Configurable the same way, via `Projects:StatePath` / `Projects:IconRoot` in app settings, should the default `$HOME/data` location ever need to move. Session assignments are removed when sessions close normally and restored when an agent re-announces a still-running session after a hub or agent restart.
