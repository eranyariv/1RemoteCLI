...
The four views live behind an eye button beside Attach rather than consuming a permanent toolbar row. Its menu closes after selection, outside interaction, or Escape, and keeps Plan visibly unavailable when there is no local task snapshot. The header similarly uses one accessible status orb instead of variable-width text: green is ready, amber is opening, owned elsewhere, or awaiting user action, red is unavailable, and a hollow grey orb is reconnecting. The full status remains the orb's accessible name and tooltip.
