...
Version 3 appended `projectId` to `SessionInfo` and added the five new hub methods and five new notifications in §5.4, on the same additive terms: an older client reads a `SessionInfo` without the field and treats every session as General, which is the correct reading of "this hub predates projects" rather than an error. The minimum supported version stayed at 1 for the same reason it did at version 2 — nothing about the wire format changed for anyone who never asks about a project.
